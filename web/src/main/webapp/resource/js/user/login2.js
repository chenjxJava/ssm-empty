...
$(document).ready(function() {
		...
		$.ajax({
			...
			success : function(data) {
				if (data != null) {
					console.info(data);
					if (data.success) {
						//window.location.href = "chooseAccount.html?mobilePhone="+mobilePhone;
						//$.post(basePath + "chooseAccount.html", {mobilephone : mobilePhone});
						//调用方法 如
						post(basePath + "chooseAccount.html", {mobilephone : mobilePhone});
					} else {
						show_pop(0, data.msg);
	...
	$("#Next_step1").click(function() {
		var id = $("#accountSelect").val();
		post(basePath + "login2.html", {id : id});
	});
