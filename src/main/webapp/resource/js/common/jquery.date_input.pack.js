DateInput = (function($) {
	...
	DateInput.prototype = {
		build: function() {
			...
			var tableShell = "<table style='margin-left:20px'><thead><tr>";
			$(this.adjustDays(this.short_day_names)).each(function() {
				tableShell += "<th>" + this + "</th>"
			});
			tableShell += "</tr></thead><tbody></tbody></table>";
			this.dateSelector = this.rootLayers = $('<div class="date_selector" style="width:250px"></div>').append(nav, tableShell).insertAfter(this.input);
		...
		selectMonth: function(date) {
			...
			if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() && this.currentMonth.getMonth() == newMonth.getMonth())) {
				...
				for (var i = 0; i <= numDays; i++) {
					...
					var currentDay1 = this.dateToString(currentDay).split('-');
					if (currentDay.getMonth() == date.getMonth()) {
							
						if (currentDay1[2].length==1) {
							var currentDay2 = currentDay1[0]+"-"+currentDay1[1]+"-"+"0"+currentDay1[2];
							dayCells += '<td class="selectable_day" date="' + currentDay2 + '">' + currentDay.getDate() + '</td>'
						}else{
						...
						if (currentDay1[2].length==1) {
							var currentDay2 = currentDay1[0]+"-"+currentDay1[1]+"-"+"0"+currentDay1[2];
							dayCells += '<td class="unselected_month" date="' + currentDay2 + '">' + currentDay.getDate() + '</td>'
						}else{
